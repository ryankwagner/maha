...
import com.yahoo.maha.core.query.hive.HiveQueryGenerator.warn
...
class HiveQueryGeneratorV2(partitionColumnRenderer:PartitionColumnRenderer, udfStatements: Set[UDFRegistration]) extends HiveQueryGeneratorCommon(partitionColumnRenderer, udfStatements) with Logging {
  ...
  override def generate(queryContext: QueryContext): Query = {
    info(s"Generating Hive query using HiveQueryGeneratorV2:, version ${version}")
    queryContext match {
...
object HiveQueryGeneratorV2 extends Logging {
  ...
  def register(queryGeneratorRegistry: QueryGeneratorRegistry, partitionDimensionColumnRenderer:PartitionColumnRenderer, udfStatements: Set[UDFRegistration]) = {
    if(!queryGeneratorRegistry.isEngineRegistered(HiveEngine, Option(Version.v2))) {
      ...
      queryGeneratorRegistry.register(HiveEngine, generator, Version.v2)
    } else {
