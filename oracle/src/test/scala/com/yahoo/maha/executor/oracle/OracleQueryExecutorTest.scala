...
class OracleQueryExecutorTest extends FunSuite with Matchers with BeforeAndAfterAll with BaseQueryGeneratorTest {
  ...
  override protected def beforeAll(): Unit = {
    ...
    oracleQueryExecutor = jdbcConnection.map(new OracleQueryExecutor(_, new NoopExecutionLifecycleListener))
    oracleQueryExecutor.foreach(queryExecutorContext.register(_))
    initDdlsAndData()
  }
  ...
  private[this] def ad_dim: PublicDimension = {
    val builder : DimensionBuilder = {
      ColumnContext.withColumnContext { implicit cc: ColumnContext =>
        ...
        Dimension.newDimension("ad_oracle", OracleEngine, LevelFour, Set(AdvertiserSchema),
          Set(
            ...
            , OracleDerDimCol("Ad Date Created", StrType(), FORMAT_DATE("{created_date}", "YYYY-MM-DD"), annotations = Set.empty)
  ...
  private[this] def ad_group_dim: PublicDimension = {
    val builder : DimensionBuilder = {
      ColumnContext.withColumnContext { implicit cc: ColumnContext =>
        import OracleExpression._
        Dimension.newDimension("ad_group_oracle", OracleEngine, LevelThree, Set(AdvertiserSchema),
          Set(
            DimCol("id", IntType(), annotations = Set(PrimaryKey))
            ...
            , OracleDerDimCol("Ad Group Start Date Full", StrType(),FORMAT_DATE("{start_time}", "YYYY-MM-dd HH:mm:ss"))
            , OracleDerDimCol("Ad Group End Date Full", StrType(),FORMAT_DATE("{end_time}", "YYYY-MM-dd HH:mm:ss"))
            , OracleDerDimCol("Ad Group Start Date", StrType(), FORMAT_DATE("{start_time}", "YYYY-MM-DD"), annotations = Set.empty)
            , OracleDerDimCol("Ad Group End Date", StrType(),FORMAT_DATE("{end_time}", "YYYY-MM-DD"), annotations = Set.empty)
            , OracleDerDimCol("Ad Group Date Created", StrType(), FORMAT_DATE("{created_date}", "YYYY-MM-DD"), annotations = Set.empty)
  ...
  def campaign_dim: PublicDimension = {
    val builder : DimensionBuilder = {
      ColumnContext.withColumnContext { implicit cc: ColumnContext =>
        import OracleExpression._
        Dimension.newDimension("campaign_oracle", OracleEngine, LevelTwo, Set(AdvertiserSchema),
          Set(
            DimCol("id", IntType(), annotations = Set(PrimaryKey))
            , DimCol("advertiser_id", IntType(), annotations = Set(ForeignKey("advertiser")))
            , DimCol("name", StrType(), annotations = Set(EscapingRequired, CaseInsensitive))
  ...
  def advertiser_dim: PublicDimension = {
    val builder : DimensionBuilder = {
      ColumnContext.withColumnContext { implicit cc: ColumnContext =>
        import OracleExpression._
        Dimension.newDimension("advertiser_oracle", OracleEngine, LevelOne, Set(AdvertiserSchema),
          Set(
            DimCol("id", IntType(), annotations = Set(PrimaryKey))
            , DimCol("name", StrType())
            , DimCol("status", StrType())
  ...
  private[this] def druidpubfact(forcedFilters: Set[ForcedFilter] = Set.empty): PublicFact = {
    ...
    ColumnContext.withColumnContext { implicit dc: ColumnContext =>
      Fact.newFact(
        "ad_group_stats_oracle", DailyGrain, DruidEngine, Set(AdvertiserSchema),
        Set(
          DimCol("ad_group_id", IntType(), annotations = Set(ForeignKey("ad_group")))
          , DimCol("campaign_id", IntType(), annotations = Set(ForeignKey("campaign")))
          , DimCol("advertiser_id", IntType(), annotations = Set(ForeignKey("advertiser")))
  ...
  def initDdlsAndData (): Unit = {
    ...
    val resultAds = jdbcConnection.get.execute(
      """
        //CREATE TABLE ad_oracle (id VARCHAR2(244), num INT, decimalValue DECIMAL, dt DATE, ts TIMESTAMP)
        CREATE TABLE ad_oracle (
          id NUMBER
    ...
    val resultAdvertiser = jdbcConnection.get.execute(
      """
        CREATE TABLE advertiser_oracle (
          id NUMBER
          , name  VARCHAR2(255 CHAR)
          , status VARCHAR2(255 CHAR)
          , created_date TIMESTAMP
    ...
    val insertSqlAdvertiser =
      """
        INSERT INTO advertiser_oracle (id, name, status, created_date, last_updated)
        VALUES (?, ?, ?, ?, ?)
      """
    ...
    insertRows(insertSqlAdvertiser, rowsAdvertiser, "SELECT * FROM advertiser_oracle")

    val insertSqlCampaign =
      """
        INSERT INTO campaign_oracle (id, name, advertiser_id, status, created_date, last_updated)
        VALUES (?, ?, ?, ?, ?, ?)
      """
    ...
    insertRows(insertSqlCampaign, rowsCampaigns, "SELECT * FROM campaign_oracle")

    val insertSqlAdGroup =
      """
        INSERT INTO ad_group_oracle (id, name, advertiser_id, campaign_id, status, created_date, last_updated)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      """
    ...
    insertRows(insertSqlAdGroup, rowsAdGroups, "SELECT * FROM ad_group_oracle")

    val insertSqlAds =
      """
        INSERT INTO ad_oracle (id, title, advertiser_id, campaign_id, ad_group_id, status, created_date, last_updated)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      """
    ...
    insertRows(insertSqlAds, rowsAds, "SELECT * FROM ad_oracle")

    val insertSqlAdsStats =
      """
        INSERT INTO ad_stats_oracle 
